...
class GFNAdaptiveSolver(nn.Module):
    def __init__(self, dim: int, s_emb_dim: int, hidden_dim: int,
        ...
        super(GFNAdaptiveSolver, self).__init__()
        self.dim = dim
    ...
    def zero(self, batch_size):
        return torch.zeros(batch_size, self.data_ndim + self.nreg, device=self.device)

    def nll_unit_gaussian(self, data, sigma=1.0):
    ...
    def nll_prior(self, state):
        ...
        return self.nll_unit_gaussian(state, np.sqrt(self.t_end) * self.g_coef)
    ...
    def f(self, t, state):
        x = torch.nan_to_num(state[:, : -self.nreg])
        pf_mean =
